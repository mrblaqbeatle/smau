...
function updateUI(balances) {
  ...
  document.getElementById('total-funds').textContent = formatUGX(balances.fixed + balances.savings + balances.investment);
}
...
function updateBalances() {
  db.ref("balances").once("value", snapshot => {
    if (snapshot.exists()) {
      ...
      data.savings += 230;
      const decrease = Math.floor(Math.random() * (420 - 240 + 1)) + 240;
      data.savings = Math.max(0, data.savings - decrease);
